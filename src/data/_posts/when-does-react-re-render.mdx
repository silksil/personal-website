...
title: 'React Re-Render Guide: when does it happen and how to deal with it?'
...
When React needs to update the app with new data, re-rendering occurs. In most cases, this occurs because users are interacting with the app or because some external data is coming through as an asynchronous request. This causes state changes, which results in re-rendering. So, the easy answer to the question of this section: **every re-render in React starts with a state change.**
...
3.  **Hook**: a change in state in the hook, will cause a re-render in the “host” component that uses it.
